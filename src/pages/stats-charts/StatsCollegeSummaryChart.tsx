...
export default function StatsCollegeSummaryChart({
  base,
  theme,
  isLoading,
  error,
  ...
  const metric = useStatsStore((s) => s.summaryMetric);
  const setMetric = useStatsStore((s) => s.setSummaryMetric);
  const splitByLevel = useStatsStore((s) => s.summarySplitByLevel);
  ...
  const { option, minWidth } = useMemo(() => {
    ...
    const titleText =
      metric === "uniqueCourses"
        ? "Number of unique courses offered by each college per semester"
        : "Enrollment by college per semester";
    const fileName =
      metric === "uniqueCourses"
        ? "unique-courses-by-college"
    ...
    const option: StatsChartOption = {
      ...
      toolbox: {
        ...
        top: 0,
        feature: {
          saveAsImage: { show: true, name: fileName },
        },
  ...
  }, [base, metric, splitByLevel, theme]);
